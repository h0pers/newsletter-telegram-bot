...
'{timedelta(days=time_data[0], hours=time_data[1], minutes=time_data[2])}', 
{data['message_thread_id'] or 'NULL'}, '{reply_chat_id}', '{data['category_id']}');
"""
...
'{reply_chat_id}', '{data['category_id']}');
"""
                await session.execute(statement=text(statement))
