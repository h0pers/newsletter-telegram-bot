...
async def periodic_newsletter_categories(query: CallbackQuery, state: FSMContext):
    try:
        async with tasks_db_session_local.begin() as session:
            statement = 'SELECT category_id FROM bot_periodic_message_tasks;'
        ...
        await state.set_state(Newsletter.choose_periodic_newsletter_category)
...
@periodic_newsletter_callback_router.callback_query(StateFilter(Newsletter.choose_periodic_newsletter_category),
                                                    Category.filter())
...
@periodic_newsletter_callback_router.callback_query(HasStates([Newsletter.edit_periodic_newsletter,
                                                                Newsletter.choose_periodic_newsletter_category]),
                                                    F.data == 'cancel')
