...
async def static_newsletter(query: CallbackQuery, bot: Bot, state: FSMContext):
...
async def delete_static_newsletter(query: CallbackQuery, callback_data: NewsletterAction, state: FSMContext):
    async with tasks_db_session_local.begin() as session:
...
@static_newsletter_callback_router.callback_query(StateFilter(Newsletter.edit_static_newsletter), F.data == 'cancel')
